  ...
  while(this.isRunning) {
    ...
    if (page === 'FriendPage') {
      ...
      this.lastVisitedPages.gameFriend = true;
        ...
        log("Calling hasEnoughHearts()...");
        // Check if there are enough hearts before proceeding
        var hasHearts = this.hasEnoughHearts();
        log("hasEnoughHearts() returned: " + hasHearts);
  
        if (!hasHearts) {
            log("Not enough hearts. Waiting...");
            this.sleep(900000); // Wait and retry after 15 min for heart
            continue; // Skip the rest of the loop until hearts are available
        }
        log("Preparing to start new game...");
        this.sleep(500);
        this.checkGameItem();
        this.sendMoneyInfo();
      ...
      log("Game is already running, skipping heart check...");
      // check again
      page = this.findPage(1, 500);
      if (page === 'GamePlaying') {
        return;
...
Tsum.prototype.hasEnoughHearts = function() {
    var img = this.screenshot();
    var heartColor = this.getColor(img, { x: 140, y: 380 }); // left most heart position
    releaseImage(img);
    
    var filledHeartColor = { r: 205, g: 74, b: 145 };
    
    if (isSameColor(heartColor, filledHeartColor, 40)) {
      log("Heart available - ready to play");
      return true;
    } else {
      log ("No red heart detected.");
      return false;
    }
    
    // log("No hearts available - waiting...");

    // log("Checking for available hearts across the entire screen...");

    // var img = this.screenshot();
    // var filledHeartColor = { r: 205, g: 74, b: 145 }; // Expected pinkish-red heart color
    // var detectedHearts = [];

    // // Scan the entire screen (adjusting ranges as needed)
    // for (var y = 200; y <= 900; y += 10) {  // Expanded Y search
    //     for (var x = 50; x <= 700; x += 10) {  // Expanded X search
    //         var heartColor = this.getColor(img, { x: x, y: y });
    //         //log("[X:" + x + ", Y:" + y + "] Detected color: " + JSON.stringify(heartColor));

    //         // Ensure detected color is a bright red heart
    //         if (heartColor.r >= 180 && heartColor.g <= 100 && heartColor.b <= 150) { 
    //             detectedHearts.push({ x: x, y: y });
    //             log("Heart detected at X: " + x + ", Y: " + y + "Detected color: " + JSON.stringify(heartColor));
    //         }
    //     }
    // }
    ...
    // if (detectedHearts.length > 0) {
    //     log("Total hearts found: " + detectedHearts.length);
    //     return false;
    // } else {
    //     log("No hearts detected.");
    //     return false;
    // }

  // SCAN FOR HEART POSITIONS
  // log("Checking for available hearts...");

  // var img = this.screenshot();
  // var filledHeartColor = { r: 187, g: 166, b: 158 };
  // var leftmostX = null;
  // var correctY = null;

  // // Try different Y-values to find the correct one
  // for (var y = 500; y <= 600; y += 10) {
  //     for (var x = 120; x <= 170; x += 5) {
  //         var heartColor = this.getColor(img, { x: x, y: y });
  //         log("[X:" + x + ", Y:" + y + "] Detected heart color: " + JSON.stringify(heartColor));

  //         if (isSameColor(heartColor, filledHeartColor, 80)) {
  //             leftmostX = x;
  //             correctY = y;
  //             log("Leftmost heart detected at X: " + leftmostX + ", Y: " + correctY);
  //             break;
  //         }
  //     }
  //     if (leftmostX !== null) break; // Stop scanning if a heart was found
  // }

  // releaseImage(img);

  // if (leftmostX !== null) {
  //     log("Leftmost heart found at X: " + leftmostX + ", Y: " + correctY);
  //     return true;
  // } else {
  //     log("No hearts detected.");
  //     return false;
